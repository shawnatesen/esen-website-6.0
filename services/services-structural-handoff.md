...
| ILIB 靜脈雷射 | PHOTOBIOMODULATION | 低能量雷射的光照療程，常見於循環與整體調理的輔助；是否適合由醫師評估 | 慢性疲勞・自律神經・睡眠・免疫調理 | 兩院區 | — |
...
1. **ILIB 靜脈雷射**：✅ **保留**（Shawn 拍板）。Josh 先前的疑問其實是「頁面上出現重複卡」，不是這項有問題——已在項目清單去重（見 §9），ILIB 本身留著、描述改衛教式。
...
5. 命名：body 統一 `ĒSEN 大安`／`ĒSEN 信義`（法定全名 `ĒSEN 伊生醫療` 只在頁尾/法律必要處）；血液檢測項目統一 **120+**。
...
Josh 之前的重複卡問題，用「一項＝一筆 CMS record、頁面用 filter 取用」的架構後基本消除。逐項確認：

| 項目 | 狀態 | 說明 |
|---|---|---|
| VO₂max | ✅ 單一 | 只在 Screen 招牌；已從 RECOVERY TECH 移出，不再兩處羅列 |
| Recovery 系列 | ✅ 單一 | 只在 Treatment（冷凍艙／紅光／高壓氧）。信義/NOX 頁的 RECOVERY TECH 是不同頁的展示，非同頁重複 |
| **自律神經** | 🔧 已修 | 原 Screen 招牌「自律神經與腦波」＋ 熱搜「情緒壓力／自律神經檢測」同頁重複 → 熱搜項改名 **情緒壓力檢測**，去掉重疊 |
| ILIB 靜脈雷射 | ✅ 單一 | 保留，只出現一次 |

**刻意的「跨類型呼應」，不是重複**（漏斗設計，請勿當成 dup 合併）：
- 熱搜「癌症風險基因檢測」 ↔ 招牌「360 基因」：單項是 SEO 入口，導流到完整方案。
- 熱搜「重金屬檢測」（檢測） ↔ Treatment「螯合療法」（療程）：一個是檢查、一個是療程，不同類型。
- 熱搜「男/女賀爾蒙檢測」（檢測） ↔ Treatment「HRT」（療程）：同上，檢測 ≠ 療程。
